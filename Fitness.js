...
document.addEventListener('DOMContentLoaded', function() {
  ...
  let appData = {
    ...
    exercises: getExercisesFromWorkoutPlan(workoutPlan),
    workoutLogs: [],
    videos: []
  };
  ...
  function showDayWorkout(day) {
    // Hide all sections and show day workout section
    const appSections = document.querySelectorAll('.app-section');
    appSections.forEach(section => {
      section.classList.remove('active-section');
    });
    ...
    if (!dayWorkoutSection) {
      console.error('Missing day-workout-section element');
      return;
    }
  ...
  function generateWorkoutTable(day) {
    ...
    for (let week = 1; week <= 6; week++) {
      ...
      // Days for this week (assuming 2 entries per week)
      for (let i = 1; i <= 2; i++) {
        ...
        // Add weight input cells for each exercise
    ...
    notesContainer.classList.add('workout-notes');
    notesContainer.innerHTML = `
      <h3>Notes</h3>
      <p>${weeklyWorkouts[day].notes}</p>
    `;
    
    // Replace any existing notes
    const existingNotes = workoutTable.parentNode.querySelector('.workout-notes');
    if (existingNotes) {
      existingNotes.parentNode.removeChild(existingNotes);
    }
    
    workoutTable.parentNode.appendChild(notesContainer);
  }
